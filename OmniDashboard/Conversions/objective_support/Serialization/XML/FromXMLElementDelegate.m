...
@synthesize mappings = mappings_, dataTypes = dataTypes_;

+ (FromXMLElementDelegate *)delegateForClass:(Class)targetClass {
	FromXMLElementDelegate *delegate = [[self alloc] init];
...
{	
    ...
    else if ([@"array" isEqualToString:[self.dataTypes objectForKey:elementName]]) {
        self.parsedObject = [NSMutableArray array];
		[self.unclosedProperties addObject:[NSArray arrayWithObjects:[self.mappings valueForKey:elementName], self.parsedObject, nil]];
		self.currentPropertyName = [self.mappings valueForKey:elementName];
	}
...
//				self.parsedObject = [[elementClass alloc] init];
        ...
        if (([self.parsedObject isKindOfClass:[NSArray class]]) ||
            ([self.mappings valueForKey:elementName]) ||
            ([[[self.parsedObject class] propertyNames] containsObject:[[self convertElementName:elementName] camelize]]))
        {
			self.currentPropertyName = [self.mappings valueForKey:elementName];//[self convertElementName:elementName];
			self.contentOfCurrentProperty = [NSMutableString string];
            
            if([self.dataTypes valueForKey:elementName])
                self.currentPropertyType = [self.dataTypes valueForKey:elementName]; 
            else
                self.currentPropertyType = [attributeDict objectForKey:@"type"];
		} 
        else 
