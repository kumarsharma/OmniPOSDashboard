...
- (void)viewDidLoad
{
    ...
    UIBarButtonItem *btnCancel = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCancel target:self action:@selector(cancelBtnAction)];
...
- (void)doneBtnAction
{
    if(self.date1)
    {
        if([self.delegate respondsToSelector: @selector(didSelectDate1:andDate2:)])
        {
            if(!self.date2)
                self.date2=self.date1;
            [self.delegate didSelectDate1:self.date1 andDate2:self.date2];
            [self dismissViewControllerAnimated:YES completion:nil];
        }
    }
}
